...
/turf/open/space/New()
	if(z==7)
		new /turf/open/floor/plating/asteroid/basalt(locate(x,y,z)) //Let's not.. create holes on this area.
		return //also grass
	if(z == 2)
		if(x >= 141 && x <= 149)
			if(y >= 256 && y <= 272)
				new /area/hallway/secondary/entry(locate(x,y,z))
				new /turf/open/indestructible(locate(x,y,z))
				new /area/hallway/secondary/entry(locate(x,y,z))
	..()
