/turf
	...
	var/waterd = 0 //1 is like 1 pixel of water.
	//var/height = 0 //0 is normal, may be used for pools of whatever.
...
/turf/Entered(mob/living/carbon/PLR)
	//may be unoptimized as fuck but works
	var/water = round(waterd)
	var/shouldswim = 0
	...
	if(water>0)
		...
		shouldswim = 0
	var/temp1 = environment.temperature-273.15
	var/temp2 = round(temp1)
	if(shouldswim==1)
		if(PLR.swimming==0)
			PLR.swimming=1
			var/msg = "The water feels fresh."
			if(temp2>35)
				msg = "The water feels warm..."
			if(temp2<6)
				msg = "The water is cold.."
			PLR.pixel_y = 0
			PLR.pixel_x = 0
			PLR << msg
	else
		PLR.swimming=0
	PLR.remove_overlay(WATER_LAYER)
	var/mutable_appearance/water_overlay = mutable_appearance('icons/mob/water_overlay.dmi', "0", -WATER_LAYER)
	PLR.overlays_standing[WATER_LAYER] = water_overlay
	water_overlay.add_overlay("[waterd]")	//we're adding icon_states of the base image as overlays
	//var/datum/gas_mixture/environment = src.return_air()
	//environment.temperature
	PLR.apply_overlay(WATER_LAYER)
