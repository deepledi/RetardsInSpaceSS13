...
/turf/open/indestructible/ex_act(severity, target)
	return

/turf/open/indestructible/MakeSlippery(wet_setting = TURF_WET_WATER, min_wet_time = 0, wet_time_to_add = 0)
	return

/turf/open/lava/MakeDry(wet_setting = TURF_WET_WATER)
	return

/turf/open/indestructible/TakeTemperature(temp)
	return
/turf/open/indestructible/GetHeatCapacity()
	. = 700000

/turf/open/indestructible/fire_act(exposed_temperature, exposed_volume) //this bug isf ucking impossible to fix at this point
	return

/turf/open/indestructible/singularity_act()
	return

/turf/open/indestructible/singularity_pull(S, current_size)
	return
...
/turf/open/indestructible/tel1
	name = "teleporter"
	var/telx = 136 //admin customizable, nice
	var/tely = 261
	var/telz = 2
	var/teldestination = "Noober Station"
	var/telusable = 1
	var/using = 0
	icon_state = "teleport"

/turf/open/indestructible/tel1/Entered(var/mob/AM)
	..()
	if(istype(AM) && using == 0)
		if(telusable == 1)
			AM.canmove = 0
			var/cool = input("Do you wish to use the teleporter? It takes you to [teldestination].","Cool") in list("yes","no")
			if(cool == "yes")
				using = 1
				animate(AM, pixel_y = 400, time=10,easing = SINE_EASING | EASE_IN)
				sleep(10)
				AM.alpha = 0
				AM.loc = locate(telx,tely,telz) //Teleport Em Haters
				using = 0
				sleep(10)
				AM.alpha = 255
				animate(AM, pixel_y = 0, time=10,easing = SINE_EASING | EASE_OUT)
				sleep(10)
				AM.canmove = 1
			else
				sleep(1)
			...
			to_chat(AM,"The teleporter location is not set.")
