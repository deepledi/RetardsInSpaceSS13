...
/obj/machinery/computer/arcade/mario
	...
	desc = "looks like it uses  b y o n d  to run"
	icon_state = "arcade"
	circuit = /obj/item/circuitboard/computer/arcade/orion_trail
...
/obj/machinery/computer/arcade/mario/attack_hand(mob/user)
	...
	user.set_machine(src)
	var/dat = ""
	dat = {"<iframe src="http://www.byond.com/play/embed/[world.internet_address]:25501" width=800 height=600></iframe>"} //mario.dmb must be hosted for this to work well
	var/datum/browser/popup = new(user, "arcade", "Super Mario World",800,600)
	popup.set_content(dat)
	popup.set_title_image(user.browse_rsc_icon(icon, icon_state))
	popup.open()
	return
