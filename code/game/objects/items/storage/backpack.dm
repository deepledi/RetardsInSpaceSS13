...
/obj/item/storage/backpack/holding/handle_item_insertion(obj/item/W, prevent_warning = 0, mob/user)
	if((istype(W, /obj/item/storage/backpack/holding) || count_by_type(W.GetAllContents(), /obj/item/storage/backpack/holding)))
		var/safety = alert(user, "I hope u know what ur doing. Go ahead.", "Put in [name]?", "Proceed", "Abort")
		if(safety == "Abort" || !in_range(src, user) || !src || !W || user.incapacitated())
			return
		investigate_log("has tried a singularity soup. Caused by [user.key]", INVESTIGATE_SINGULO)
		to_chat(user, "<span class='danger'>The Bluespace interfaces of the two devices catastrophically malfunction!</span>")
		qdel(W)
		var/obj/item/storage/toolbox/bruh = new /obj/item/storage/toolbox (get_turf(src))
		//singulo.energy = 300 //should make it a bit bigger~
		bruh.force = 999999
		user << "<font color='red'>U are now officall,y robust, grab the toolbox and become a killer machine, it instakills everyone."
