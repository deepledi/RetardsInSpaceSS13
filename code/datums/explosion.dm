...
/datum/explosion/New(atom/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog, ignorecap, flame_range, silent, smoke)
	...
	//Lmao this is unfunny
	if(!ignorecap)
		devastation_range = 0 //min(GLOB.MAX_EX_DEVESTATION_RANGE * cap_multiplier, devastation_range) //wonder if devastation is that big of deal
		//No Holes Now. Too Griefy.
		heavy_impact_range = min(1, heavy_impact_range) //no fag
		//light_impact_range =  //min(GLOB.MAX_EX_LIGHT_RANGE * cap_multiplier, light_impact_range) uh ya u can do supper light impact with this. no cap i guess as it harms nobody
		if(light_impact_range > 10)
			light_impact_range = 10
		flash_range = min(20, flash_range)
		flame_range = min(10, flame_range)
	...
	var/z0 = epicenter.z
	spawn()
		var/obj/Mode7/Explosion/EXA = new/obj/Mode7/Explosion() //Same.......
		EXA.loc = locate(epicenter.x,epicenter.y,epicenter.z)
		EXA.poop = max_range*3 //double it.
	//message_admins("explosion created ([epicenter.x],[epicenter.y],[epicenter.z]) effect")
