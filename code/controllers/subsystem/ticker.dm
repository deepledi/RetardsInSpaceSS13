...
SUBSYSTEM_DEF(ticker)
	...
	var/gametime_offset = 72000 // equal to 12 hours, making gametime at roundstart 12:00:00
...
/datum/controller/subsystem/ticker/Initialize(timeofday)
	...
	login_music = "sound/ambience/title[rand(1,5)].ogg" //See below, this changed.
...
/client/proc/playtitlemusic(vol = 85)
	...
	if(prefs && (prefs.toggles & SOUND_LOBBY))
		SEND_SOUND(src, sound("sound/ambience/title[rand(1,5)].ogg", repeat = 0, wait = 0, volume = vol, channel = CHANNEL_LOBBYMUSIC)) // MAD JAMS
...
/datum/controller/subsystem/ticker/proc/PostSetup()
	...
	call("ByondPOST.dll", "send_post_request")("[discordvar]", " { \"content\" : \"Round [GLOB.round_id ? "#[GLOB.round_id]:" : "of"] [hide_mode ? "secret":"[mode.name]"] has started[allmins.len ? ",":" with no active admins online!,"] You can join with the ip [world.internet_address]:[world.port] \" } ", "Content-Type: application/json")
	//send2irc("Server", "Round [GLOB.round_id ? "#[GLOB.round_id]:" : "of"] [hide_mode ? "secret":"[mode.name]"] has started[allmins.len ? ".":" with no active admins online!"] You can join with the ip [world.internet_address]:[world.port]")
	setup_done = TRUE
	var/msg = "<span class='warning'>Spectator visibility activated.</span>"
...
/datum/controller/subsystem/ticker/proc/declare_completion()
	...
	world << sound(null,channel=CHANNEL_ESCAPESONG)

	to_chat(world, "<BR><BR><BR><FONT size=3><B>The round has ended.</B></FONT>")

	for(var/client/C in GLOB.clients)
		SEND_SOUND(C, sound("sound/Ending[rand(1,2)].ogg", repeat = 0, wait = 0, volume = 100, channel = CHANNEL_LOBBYMUSIC)) // MAD JAMS
		//C.playtitlemusic(40)
	//NIGGA
	call("ByondPOST.dll", "send_post_request")("[discordvar]", " { \"content\" : \"Round ended at [time_stamp()]. Admin reboot can be done now. Shift Duration: [DisplayTimeText(world.time - SSticker.round_start_time)]\" } ", "Content-Type: application/json")
	...
	for(var/i in GLOB.mob_list)
		...
		if(Player.mind && !isnewplayer(Player))
			if(Player.stat != DEAD && !isbrain(Player))
				Player << 'survivor.ogg'
				to_chat(Player, {"<font size="12"><font color="green">Good job!</font></font>"}) //GG HE USURVIVOR
				num_survivors++
	...
	to_chat(world, "<BR>[GLOB.TAB]Round duration: <B>[DisplayTimeText(world.time - SSticker.round_start_time)]</B>")
...
/datum/controller/subsystem/ticker/proc/send_news_report()
	var/news_message
	var/news_source = "Fake News Network"
	switch(news_report)
...
/datum/controller/subsystem/ticker/Shutdown()
	if(!round_end_sound)
		round_end_sound = 'sound/roundend/complete.ogg'
