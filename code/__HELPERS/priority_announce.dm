/proc/priority_announce(text, title = "", sound = 'sound/ai/attention.ogg', type , sender_override)
	...
	if(type == "Priority")
		announcement += "<font color = white><h1 class='alert'>Priority Announcement</h1>"
		if (title && length(title) > 0)
			announcement += "<font color = white><br><h2 class='alert'>[html_encode(title)]</h2>"
	else if(type == "Captain")
		announcement += "<font color = white><h1 class='alert'>Captain Announces</h1>"
		GLOB.news_network.SubmitArticle(text, "Captain's Announcement", "Station Announcements", null)

	else
		if(!sender_override)
			announcement += "<font color = white><h1 class='alert'>[command_name()] Update</h1>"
		else
			announcement += "<font color = white><h1 class='alert'>[sender_override]</h1>"
		if (title && length(title) > 0)
			announcement += "<font color = white><br><h2 class='alert'>[html_encode(title)]</h2>"
