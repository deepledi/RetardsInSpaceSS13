...
/client/proc/cmd_admin_rejuvenate(mob/living/M in GLOB.mob_list)
	...
	var/msg = "<span class='danger'>Admin [key_name_admin(usr)] has healed [key_name_admin(M)]!</span>"
	message_admins(msg)
	admin_ticket_log(M, msg)
...
/client/proc/cmd_admin_create_centcom_report()
	...
	var/input = input(usr, "Please enter anything you want.", "What?", "") as message|null
	if(!input)
		return
...
/client/proc/admin_call_shuttle()
	...
	set category = "Admin"
	set name = "Call Shuttle (Owner Only)"
	...
	if(src.key == world.host)
		...
		to_chat(src, "Only the host can call the shuttle.")
		message_admins("<span class='adminnotice'>[key_name_admin(usr)] tried to call the shuttle but did not have enough permission.</span>")
	return

/client/proc/admin_cancel_shuttle()
	set category = "Admin"
	set name = "Cancel Shuttle (Owner Only)"
	if(!check_rights(0))
		return
	...
	if(EMERGENCY_AT_LEAST_DOCKED)
		return
	//message_admins("<span class='adminnotice'>[key_name_admin(usr)] is recalling the shuttle.</span>")
	if(src.key == world.host)
		...
		to_chat(src, "Only the host can recall the shuttle.")
		message_admins("<span class='adminnotice'>[key_name_admin(usr)] tried to recall the shuttle but did not have enough permission.</span>")
	return
