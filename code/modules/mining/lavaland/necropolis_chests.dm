...
/obj/item/fireball_staff
	name = "staff of falling fireballs"
	desc = "Spam on em."
...
/obj/item/fireball_staff/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	..()
	...
	if(target in view(user.client.view, get_turf(user)))
		...
		if(!istype(T))
			...
			message_admins("[key_name_admin(user)] fired the falling fireball staff at [get_area(target)]. [ADMIN_COORDJMP(T)]")
...
/obj/effect/temp_visual/lavastaff
	icon_state = "supper nigger no warn"
	duration = 50
