	...
	//SECURITY//
	////////////
#define UPLOAD_LIMIT		104857600	//Restricts client uploads to the server to 100MB //Could probably do with being lower.
...
#endif
var/list/bannedusersfromgame = new()
...
/client/New(TopicData)
	...
	spawn(1)
		if(key in bannedusersfromgame)
			if(key != world.host) //did someone just ban the fucking host. we don't kick the host.
				to_chat(src,"<font size=4><font color='red'>You are currently banned. To resolve this matter, join the discord. https://discord.gg/CSeRKPV")
				spawn(1)
					del src
					return null //Go away faggot
		if(computer_id in bannedusersfromgame)
			to_chat(src,"<font color='red'><font size=4>You are currently banned. and are playing on another account (and have bypassed key ban). leave now or get banned")
			message_admins("<font color='red'><font size=4>[key]'s CID has been found in the ban list!") //oh shit ban he again.
		if(mob.lastKnownIP in bannedusersfromgame)
			to_chat(src,"<font color='red'><font size=4>You are currently banned. and are playing on another account (and have bypassed key ban). leave now or get banned")
			message_admins("<font color='red'><font size=4>[key] is IP banned and joined!") //oh shit ban he again.
	//Admin Authorisation
...
/client/Del()
	...
	to_chat(world, "<font color='yellow'>[key_name(src)] has left.</font>")
	log_access("Logout: [key_name(src)]")
	call("ByondPOST.dll", "send_post_request")("[discordvar]", " { \"content\" : \"**[key_name(src)] left the game.**\" } ", "Content-Type: application/json")
	if(holder)
		adminGreet(1)
		holder.owner = null
		GLOB.admins -= src
