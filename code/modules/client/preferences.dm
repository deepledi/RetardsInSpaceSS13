...
/datum/preferences
	...
	var/tmp/old_be_special = 0			//Bitflag version of be_special, used to update old savefiles and nothing more
										//If it's 0, that's good, if it's anything but 0, the owner of this prefs file's antag choices were,f
	...
	var/datum/species/pref_species = new /datum/species/angel()	//Mutant race u gay lol
...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	if(!path)
		dat += "<div class='notice'>Register an account to save your data!</div>"
	...
	switch(current_tab)
		if (0) // Character Settings#
			...
			dat += "<a href='?_src_=prefs;preference=name;task=random'>Random Name Choice</A> "
			dat += "<a href='?_src_=prefs;preference=name'>Always use CKEY for Name: [be_random_name ? "Yes" : "No"]</a> ([parent.key])<BR>"

			dat += "<b>Name:</b> "
			dat += "<a href='?_src_=prefs;preference=name;task=input'>[real_name]</a> <a href='?_src_=prefs;preference=poopsggg;task=input'>Use ckey</a><BR>"

			dat += "<b>Gender:</b> <a href='?_src_=prefs;preference=gender'>[gender == MALE ? "Male" : "Female"]</a><BR>"
			dat += "<b>Age:</b> <a href='?_src_=prefs;preference=age;task=input'>[age]</a><BR>"
			dat += "<b>Max Health : </b> <a href='?_src_=prefs;preference=poops;task=input'>[poop]</a><BR>"
			dat += "<b>Antag Role : </b> <a href='?_src_=prefs;preference=poopsgg;task=input'>[poopggantagrole]</a><BR>"

			//dat += "<font color='red'>did you know? nobody fucking cares how OP your character is. so dont brag.</font><BR>"
			...
			dat += "<a href ='?_src_=prefs;preference=cyborg_name;task=input'><b>Cyborg:</b> [custom_names["cyborg"]]</a><BR><BR></td>"
			...
			dat += "<b>Pee Rate:</b><BR><a href='?_src_=prefs;preference=damn;task=input'>[peeRateDefault]</a><BR>"
			...
			if((MUTCOLORS in pref_species.species_traits) || (MUTCOLORS_PARTSONLY in pref_species.species_traits))
				if(pref_species.canchangecolor==1)
		...
		if (1) // Game Preferences
			...
			//dat += "<b>GUI (Only one theme):</b> <a href='?_src_=prefs;task=input;preference=ui'>[UI_style]</a><br>"
			dat += "<b>Controls:</b> <a href='?_src_=prefs;preference=hotkeys'>[(hotkeys) ? "Hotkeys" : "Default"]</a><br>"
			...
			dat += "<b>Parallax (Space) Recommended Insane:</b> <a href='?_src_=prefs;preference=parallaxdown' oncontextmenu='window.location.href=\"?_src_=prefs;preference=parallaxup\";return false;'>"
			switch (parallax)
				if (PARALLAX_LOW)
					dat += "Low"
				if (PARALLAX_MED)
	...
	var/datum/browser/popup = new(user, "preferences", "<div align='center'>Character Setup</div>", 800, 770)
			...
			switch(href_list["preference"])
				...
				if("name")
					...
					if(new_name)
						real_name = new_name
				if("age")
					var/new_age = input(user, "Choose your character's age (ANY)", "Character Preference") as num|null
					if(new_age)
						age = new_age
				if("poops")
					var/poo = input(user, "How much HP you will have (1-150)", "Character Preference") as num|null
					if(poo)
						if(poo > 150)
							poo = 150
						if(poo < 1)
				...
				if("poopsgg")
					var/poo = input(user, "Antag role", "Character Preference") in list("Civilian","Traitor","Changeling","Devil","Wizard")
					if(poo)
				...
				if("species")
					...
					if(result)
						...
						var/datum/species/speciesc = new newtype()
						if(speciesc.warningwhitelist==0)
							pref_species = new newtype()
							//Now that we changed our species, we must verify that the mutant colour is still allowed.
							var/temp_hsv = RGBtoHSV(features["mcolor"])
							if(features["mcolor"] == "#000" || (!(MUTCOLORS_PARTSONLY in pref_species.species_traits) && ReadHSV(temp_hsv)[3] < ReadHSV("#7F7F7F")[3]))
								features["mcolor"] = pref_species.default_color
							if(pref_species.canchangecolor==0)
								features["mcolor"] = pref_species.default_color //Use their default color if they have any
						else
							var/found = 0
							var/list/whitelist = world.file2list("config/allowed.txt")
							to_chat(user, "<p style='color:green'>Checking access for [user.key]... Please wait.</p>")
							if(user.key in whitelist)
								to_chat(user, "<p style='color:green'>Access Granted for [user.key].</p>")
								found = 1
							if(found==0)
								to_chat(user, "<p style='color:red'>Req Failed. [speciesc.warningmessage]</p>")
							else //Furry whitelist.
								//Allowed to play.
								/*
								You may be wondering. Why do we have this whitelist even?

								Guess why.
								Have you ever heard of vulpy or riki station 4?

								That's why. Noober84555 and Borksin requested this whitelist so we don't see liggers going around or vulpkanins doing ERP on dorms.
								Or xeno-vulps. Xenos are removed anyways, so it's not worth it for them. Thats all what i'm gonna say. Thanks for listening to this advice. Happy coding!
								*/
								pref_species = new newtype()
								//Now that we changed our species, we must verify that the mutant colour is still allowed.
								var/temp_hsv = RGBtoHSV(features["mcolor"])
				...
				if ("clientfps")
					...
					var/desiredfps = input(user, "Choose your desired fps.[version_message]\n(0 = synced with server tick rate (currently:[world.fps]) recommended 120)", "Character Preference", clientfps)  as null|num
...
/datum/preferences/proc/copy_to(mob/living/carbon/human/character, icon_updates = 1)
	if(be_random_name)
		real_name = parent.key
	//list("Civilian","Traitor","Changeling","Devil")
	spawn()
		sleep(10) //good
		if(poopggantagrole == "Traitor")
			character.mind.make_Traitor()
		if(poopggantagrole == "Changeling")
			character.mind.make_Changling()
		if(poopggantagrole == "Devil")
			add_devil(character, FALSE)
		if(poopggantagrole == "Wizard")
			character.mind.make_Wizard()
	...
	if(pref_species.id == "novakid")
		character.eye_color = "000" //fix to remove some stuff
	else
	...
	if(pref_species.id == "novakid")
		character.hair_style = "" //no hair allowed
	else
	...
	character.health = poop
	character.maxHealth = poop
