...
/mob/proc/AIize(transfer_after = TRUE)
	...
	if(!transfer_after)
		mind.active = FALSE
	to_chat(src,"Spawning AI.. please adminhelp if this does not work at all. And you don't appear at the correct position.")
	. = new /mob/living/silicon/ai(locate(26,198,2), null, src)

	qdel(src)

/mob/living/carbon/human/proc/Robotize(delete_items = 0, transfer_after = TRUE, var/forcegg = 0)
	to_chat(src,"Converting to cyborg... Please adminhelp if this does not work at all.")
	if (notransform && forcegg == 0)
		return
	for(var/obj/item/W in src)
		if(delete_items)
			qdel(W)
...
//human -> alien
/mob/living/carbon/human/proc/Alienize() //NO FETISH A ALLOWED
	qdel(src)
