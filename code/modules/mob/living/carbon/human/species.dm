...
/datum/species
	...
	var/warningmessage = "You need to be on the whitelist to play this race. Adminhelp this issue."
	var/warningwhitelist = 0
	var/hair_color	// this allows races to have specific hair colors... if null, it uses the H's hair/facial hair colors. if "mutcolor", it uses the H's mutant_color
	var/hair_alpha = 255	// the alpha used by the hair. 255 is completely solid, 0 is transparent.
	var/racedesc = "a fucking retard" //unused..
...
/proc/generate_selectable_species()
	GLOB.roundstart_races += "angel" //ROBLOX enchantix school remake the default race
	GLOB.roundstart_races += "dullahan" //Headless Man

	GLOB.roundstart_races += "novakid" //the gas men
	GLOB.roundstart_races += "vampire" //they're back!
	GLOB.roundstart_races += "pod" //plant boys
	GLOB.roundstart_races += "shadow" //edgy boys
	GLOB.roundstart_races += "skeleton" //bone boys
	GLOB.roundstart_races += "jelly" //jelly boys

	GLOB.roundstart_races += "plasmaman" //gas men v2
	GLOB.roundstart_races += "abductor" //AYYLIENS

	GLOB.roundstart_races += "vulpkanin" //furries
	GLOB.roundstart_races += "lizard" //furries
	GLOB.roundstart_races += "flamingo" //animals
	//disable the boys
...
/datum/species/proc/handle_digestion(mob/living/carbon/human/H)
	...
	if(H.nutrition > NUTRITION_LEVEL_FAT)
		...
		if(H.metabolism_efficiency != 1.25 && (H.dna && H.dna.species && !(NOHUNGER in H.dna.species.species_traits)))
			to_chat(H, "<span class='notice'>You feel full.</span>")
			H.metabolism_efficiency = 1.25
	else if(H.nutrition < NUTRITION_LEVEL_STARVING + 50)
		if(H.metabolism_efficiency != 0.8)
			to_chat(H, "<span class='notice'>You are starving. It hurts.</span>")
		H.metabolism_efficiency = 0.8
		H.adjustStaminaLoss(5)
	else
		if(H.metabolism_efficiency == 1.25)
			to_chat(H, "<span class='notice'>You no longer feel full.</span>")
		H.metabolism_efficiency = 1
