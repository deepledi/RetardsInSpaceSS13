...
/mob/living/carbon/human/Stat()
	..()
	if(PeeRate != 0)
		if(statpanel("Hygiene Stats"))
			var/PeeNeed = min(PeeAmount,100)
			var/PeeNeedFinal = max(PeeNeed,0)
			stat("Need To Pee : %[PeeNeedFinal]")
			stat("This is currently not finished. Report any bugs to Noober84555")

/mob/living/carbon/human/verb/peepee()
	set name = "Pee (Hygiene)"
	pee()

/mob/living/carbon/human/proc/handle_pee()
	if(PeeAmount >= PeeMax)
		//dangit
		pee()

/mob/living/carbon/human/proc/pee()
	if(PeeRate != 0)
		if(PeeAmount > 15) //above %15

			var/FOUNDTOIL = 0
			for(var/obj/structure/toilet/GG in view(1,src.loc))
				FOUNDTOIL = FOUNDTOIL + 1
			for(var/obj/structure/urinal/GG in view(1,src.loc))
				FOUNDTOIL = FOUNDTOIL + 1
			playsound(src.loc, "pee.ogg", 80, 1, 1)


			if(FOUNDTOIL > 0)
				to_chat(src,"<font color='yellow'>You pee on the urinal/toilet.")
				src.emote("me",1,"pees on the toilet/urinal!")
			else
				to_chat(src,"<font color='red'>Mason, what the fuck? Peeing in the carpet? Damn it.")
				src.emote("me",1,"pees themselves!")
				var/obj/effect/decal/cleanable/pee/p = new(locate(x,y,z))
				p.color = PeeColor
				p.pp = PeeAmount
			PeeAmount = -15
		else
			to_chat(src,"<font color='red'>ow that hurts. i cannot pee.")
...
/mob/living/carbon/human/check_breath(datum/gas_mixture/breath, var/isinternals = 0)
	//WATER DROWNING
