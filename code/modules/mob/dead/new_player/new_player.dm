...
/mob/dead/new_player/proc/new_player_panel()
	var/output = "<center><p><a href='byond://?src=[REF(src)];show_preferences=1'>Create Character</a></p>"
	//output += "<br><br><font color='red'>please stop roleplaying as xenos<br>for borksin's sanity<br>you're literally worser than nitro<br>if you do rp as them.</font><br>penalty : ban"
	if(SSticker.current_state <= GAME_STATE_PREGAME)
		//switch(ready)
			/*if(PLAYER_NOT_READY)
				output += "<p>\[ [LINKIFY_READY("Ready", PLAYER_READY_TO_PLAY)] | <b>what the fuck are you doing</b> | [LINKIFY_READY("Observe", PLAYER_READY_TO_OBSERVE)] \]</p>"
			if(PLAYER_READY_TO_PLAY)
				output += "<p>\[ <b>im ready</b> | [LINKIFY_READY("Not Ready", PLAYER_NOT_READY)] | [LINKIFY_READY("Observe", PLAYER_READY_TO_OBSERVE)] \]</p>"
			if(PLAYER_READY_TO_OBSERVE)
				output += "<p>\[ [LINKIFY_READY("Ready", PLAYER_READY_TO_PLAY)] | [LINKIFY_READY("Not Ready", PLAYER_NOT_READY)] | <b> spectate </b> \]</p>"*/
	else
		//output += "<p><a href='byond://?src=[REF(src)];manifest=1'>View the Crew Manifest</a></p>"
		output += "<p><a href='byond://?src=[REF(src)];late_join=1'>Play</a></p>"
		output += "<p>[LINKIFY_READY("Spectate", PLAYER_READY_TO_OBSERVE)]</p>"
	...
	//src.stop_sound_channel(CHANNEL_LOBBYMUSIC)
	return
	...
	if(statpanel("Lobby"))
		...
		if(SSticker.current_state == GAME_STATE_PREGAME)
			...
			if(time_remaining > 0)
				...
				stat("Time To Start:", "damn")
			else
				stat("Time To Start:", "dang")

			stat("Players:", "[SSticker.totalPlayers]")
			//if(client.holder)
				//stat("amount of niggers in my store:", "[SSticker.totalPlayersReady]")
...
/mob/dead/new_player/proc/LateChoices()
	...
	for(var/datum/job/job in SSjob.occupations)
		if(job && IsJobAvailable(job.title))
			...
			dat += "<a class='[position_class]' href='byond://?src=[REF(src)];SelectedJob=[job.title]'>[job.title]</a><br>"
