...
/mob/living/verb/ghost()
	...
	if(stat == DEAD)
		...
		var/response = alert(src, "Are you sure you want to ghost? You will be able to return any time.","Are you sure you want to ghost?","Ghost","Stay in body")
		if(response != "Ghost")
			return	//didn't want to ghost after-all
		ghostize(1)						//u can go back
	return
...
/mob/dead/observer/verb/reenter_corpse()
	...
	if(mind.current.key && copytext(mind.current.key,1,2)!="@")	//makes sure we don't accidentally kick any clients
		to_chat(usr, "<span class='warning'>You have kicked [mind.current.key] from your body.</span>")
