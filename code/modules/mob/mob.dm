/mob
	use_tag = TRUE
	var/swimming = 0 //this makes some stuff happen
...
/mob/Stat()
	...
	if(statpanel("Radar"))
		stat("Position:","[x],[y]")
		var/ee = 0
		for(var/mob/living/i in obounds(src, 10*32)) //LETS KEEP IT SMALL PLZ
			if(i.health > 0)
				stat("[i.name] [i.health]/[i.maxHealth]","Distance : [abs(x - i.x)],[abs(y - i.y)]") //let's make it show 1,1 when -1,-1 for example
			else
				stat("[i.name] Critical condition!","Distance : [abs(x - i.x)],[abs(y - i.y)]")
			ee = ee + 1
		stat("Detected lifesigns:","[ee]")
